...
class DataPreprocessor():
    def __init__(self,args):
        ...
        self.num_neighbor = args.num_neighbor if args.num_neighbor!=0 else None
        self.boxiness_coef = args.boxiness
        #Data specs
        self.data_Dir = "project_data/BraTS20_data/"

        self.output_Dir = os.path.expanduser(args.output_Dir if args.output_Dir else f"{Filepaths.PROCESSED_DATA_DIR}_{args.num_node}_{args.boxiness}_{args.num_neighbor}")
        self.mri_prefix = "BraTS20_Training"
        self.modality_extensions=args.modality_extensions
        self.label_extension = args.label_extension
        self.include_label = args.label_extension is not None
        self.all_ids,self.id_to_fp = self.get_all_mris_in_dataset()

        data_stat=self.compute_dataset_stats() if STANDARDIZATION_STATS is None else STANDARDIZATION_STATS
    ...
    def get_all_mris_in_dataset(self):
        mri_folder = glob.glob(f"{self.data_Dir}**/{self.mri_prefix}*/",recursive=True)
        mri_folder = self.remove_incomplete_mris(mri_folder)

        scan_dic = {fp.split("\\")[1]:fp for fp in mri_folder}
        if(len(mri_folder)==0):
            print("Double check input path.")
        print(f"Found {len(mri_folder)} MRIs")

        return list(scan_dic.keys()),scan_dic

    def remove_incomplete_mris(self,mri_folder):
        #if there are any you want to ignore just add them to this list
        removed_mris=[]
        return [fp for fp in mri_folder if fp.split("/")[-2] not in removed_mris]

    def get_standardized_image(self, scan_full_path):
        img_data = nifti_io.read_in_patient_sample(scan_full_path,self.modality_extensions)
        crop_idx = determine_brain_crop(img_data)
        crop_data = img_data[crop_idx]
        if(self.label_extension):
            label_data = nifti_io.read_in_labels(scan_full_path,self.label_extension)
            cropped_labels = label_data[crop_idx]
            standardized_label=swap_labels_from_brats(cropped_labels)
        else:
            standardized_label=None


        normalized_dataset = normalize_img(crop_data)
        standardized_dataset = standardize_img(normalized_dataset,self.Dataset_mean,self.Dataset_std)

        return standardized_dataset,standardized_label,crop_idx
    ...
    def compute_dataset_stats(self):
        ...
        for mri_id in self.all_ids:
            ...
            image = nifti_io.read_in_patient_sample(mri_path,self.modality_extensions)
            lab = nifti_io.read_in_labels(mri_path,self.label_extension)
            if(len(image.shape)>3):
                healthy_tissue_mask = np.logical_and(image[:,:,:,0]>0.001,lab==0)
                image=image[healthy_tissue_mask]
                image = normalize_img(image,is_flat=True)
                meanu = np.mean(image,axis=0)
        ...
        Dataset_mean = np.median(img_mean,axis=0)
        Dataset_deviation = np.median(img_deviation,axis=0)
    ...
    def process_next_sample(self,mri_id):
        ...
        nx_graph,node_feats,region_img = img2graph(image_data,label_data,self.num_node,self.boxiness_coef,self.num_neighbors)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-k', '--num_neighbors', default=0, help='How many neighbors each node has in the adjacency matrix. Passing 0 will create adjacency matrix based strictly on contiguous supervoxels',type=int)
    ...
    parser.add_argument('-m','--modality_extensions', nargs="+", default=["_flair.nii","_t1.nii","_t1ce.nii","_t2.nii"],help="The file extensions for each desired modality. Accepts a variable amount of modalities. Ensure consistent order.")
