...
def load_net_and_weights(weight_file):
    ...
    att_heads = [5, 6, 4, 5, 4, 6, 4, 4]
    residuals = [False, False, True, False, False, False, False, False]
...
def save_predictions(net,dataset,save_format='preds'):
    global device
    net = net.to(device)
...
def save_voxel_logits(mri_id,Dataset,node_logit):
    global output_Dir
    node_logit=node_logit.detach().cpu().numpy()
    supervoxel_partitioning = Dataset.get_supervoxel_partitioning(mri_id)
    #add placeholder logits for healthy tissue
    node_logits = np.concatenate([node_logit,DEFAULT_BACKGROUND_NODE_LOGITS])
...
def save_voxel_preds(mri_id,Dataset,node_logit):
    global output_Dir
    ...
    predicted_voxel = uncrop_to_brats_size(raw_data_crop,predicted_voxel)
    predicted_voxel = swap_labels_to_brats(predicted_voxel)
    nifti_io.save_as_nifti(predicted_voxel,f"{output_Dir}{os.sep}{mri_id}.nii.gz")
...
if __name__ == '__main__':
    ...
    if args.output_dir:
        ...
        output_dir = Filepaths.GNN_LOGIT_DIR if args.save_format=="preds" else Filepaths.PRED_DIR
