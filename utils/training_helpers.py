...
def create_run_progress_file(fp,model_type,hp):
    with open(fp, "w") as f:
        ...
        f.write("Fold\tLoss\tWT_Volex_Dice\tCT_Volex_Dice\tET_Volex_Dice\tWT_HD95\tCT_HD95\tET_HD95\n\n")
...
def update_progress_file(fp,description,loss,dices):
    with open(fp, "a") as f:
        f.write(f"{description}\t{loss}\t{dices[0]}\t{dices[1]}\t{dices[2]}\t{dices[3]}\t{dices[4]}\t{dices[5]}\n")
...
def train_on_fold(model,checkpoint_dir,n_epoch,run_name,fold):
    ...
    for i in range(1,n_epoch+1):
        epoch_loss, f1_score = model.run_epoch()
        #TODO: revert
        if(i%1==0):
            print(f"________Epoch {i}_________f1_Socre_________")
            print(epoch_loss, " | ",f1_score)
