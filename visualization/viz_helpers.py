...
def define_label_cmap(continuous_labels=False):
    if continuous_labels:
        ...
        lut = np.array([
            ...
            [0, 0, 0],
            [219, 185, 13]])/255
    print("Healthy: Green (or transparent), Edema: Red, NET (Core): Blue, ET (Enhancing): Yellow")
...
def get_data_for_plotting(data_folder,seg_folder,mri_id,mod1_ext,mod2_ext,mod3_ext,mod4_ext,read_labels=True):
    ...
    mod3 = read_nifti(f"{data_folder}/{mri_id}/{mri_id}{mod3_ext}", np.float32)
    mod3 = mod3 / np.max(mod3)
    mod4 = read_nifti(f"{data_folder}/{mri_id}/{mri_id}{mod4_ext}", np.float32)
    mod4 = mod4 / np.max(mod4)
    ...
    if(read_labels):
        labels = read_nifti(f"{data_folder}/{mri_id}/{mri_id}_seg.nii",np.int16)
    ...
    mod3=mod3[30:220,30:220]
    mod4=mod4[30:220,30:220]
    overlaid_gt=overlaid_gt[30:220,30:220]

    return mod1,mod2,mod3,mod4,overlaid_preds,overlaid_gt
